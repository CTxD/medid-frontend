...
class _CamPageState extends State<CamPage> {
  CamBloc get _camBloc => widget.camBloc == null
      ? BlocProvider.of<CamBloc>(context)
      : widget.camBloc;
  CameraPreviewWidget get _cameraPreview => widget.cameraPreview == null
      ? CameraPreviewWidget()
        ...
        Navigator.of(context).push(MaterialPageRoute(
          builder: (context) => ResultPage(
              resultBloc: ResultBloc(
                  createFile: (String path) => File(path),
                  pillRepository: PillRepository(
                      pillApiClient: PillApiClient(httpClient: http.Client()))),
              imageFilePath:
                  (_camBloc.currentState as CamPictureTaken).imageFilePath),
        ));
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(title: Text("Identificer Pille")),
                    ...
                    Transform.scale(
                      ...
                      child: AspectRatio(
                          ...
                          child: _cameraPreview
                              .getCameraPreview(state.controller)),
                    ),
                    Center(
                      child: CustomPaint(
...
class ShapesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Top of the rect
    canvas.drawRect(Rect.fromLTRB(0, 0, size.width, startY), paint);

    // Middle left
    canvas.drawRect(Rect.fromLTRB(0, startY, startX, endY), paint);

    // Middle right
    canvas.drawRect(Rect.fromLTRB(endX, startY, size.width, endY), paint);

    // Bottom of rect
    canvas.drawRect(
        Rect.fromLTRB(0, endY - .1, size.width, size.height), paint);
